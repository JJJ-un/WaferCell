...
const FEELING_OPTIONS = [
  { value: 'CALM', label: '차분함', emoji: '😃', desc: '이성적이고 평온함', color: 'border-blue-500 bg-blue-50/60 text-blue-600' },
  { value: 'GREEDY', label: '탐욕', emoji: '🤑', desc: '추격 매수 및 과열', color: 'border-emerald-500 bg-emerald-50/60 text-emerald-600' },
  { value: 'FEAR', label: '공포', emoji: '😨', desc: '불안 및 패닉 셀링', color: 'border-rose-500 bg-rose-50/60 text-rose-600' },
  { value: 'NEUTRAL', label: '평온', emoji: '😐', desc: '무감정 및 원칙 매매', color: 'border-slate-500 bg-slate-100/60 text-slate-600' }
];
...
export const JournalDrawer = ({ isOpen, onClose, ticker, initialDate, existingJournal }: JournalDrawerProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end select-text text-slate-800">
      {/* 백드롭 (클릭 시 닫기) */}
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col gap-5 overflow-y-auto pr-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold text-slate-600">날짜</label>
              <input 
                type="date"
                value={date}
                onChange={(e) => setDate(e.target.value)}
                className="bg-slate-50 border border-slate-200/80 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none focus:bg-white focus:border-blue-500 transition w-full font-medium"
                required
          ...
          {/* 종목 표시 */}
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-slate-600">종목</label>
            <div className="bg-slate-50 border border-slate-200/80 rounded-lg px-3 py-2 text-sm text-slate-800 font-extrabold flex items-center gap-2">
              <span className="w-2.5 h-2.5 rounded-full bg-blue-505 animate-pulse"></span>
              {ticker.toUpperCase()}
            </div>
          </div>
                ...
                return (
                  <button
                    ...
                    className={`py-1.5 text-xs font-bold rounded-lg border transition cursor-pointer
                      ${actionType === type 
                        ? 'border-blue-500 bg-blue-55 text-blue-600 font-extrabold shadow-xs' 
          ...
          {actionType !== 'MEMO' && (
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-bold text-slate-600">매매 단가 (USD)</label>
                <input 
                  type="number"
                  step="any"
                  value={price}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-bold text-slate-600">매매 수량</label>
                <input 
                  type="number"
                  step="any"
                  value={quantity}
          ...
          {/* 당시 심리 상태 */}
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-slate-600">당시 심리 상태</label>
          ...
          {/* 투자 일지 및 복기 메모 */}
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-slate-600">투자 일지 및 복기 메모</label>
            <textarea 
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder="매매의 명확한 근거와 오늘의 생각들을 상세히 기록해 보세요..."
              className="bg-slate-50 border border-slate-200/80 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none focus:bg-white focus:border-blue-500 transition h-32 resize-none leading-relaxed placeholder-slate-400 font-medium"
              maxLength={3000}
              disabled={!!existingJournal}
            />
          </div>
          ...
          <div className="mt-auto pt-4 flex flex-col gap-2">
            {!existingJournal ? (
              <button
                type="submit"
                disabled={isSaving}
                className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-2.5 rounded-lg text-sm transition cursor-pointer disabled:opacity-50"
              >
                {isSaving ? '저장 중...' : '기록 저장'}
              </button>
            ) : (
              <button
                ...
                className="w-full bg-rose-50 hover:bg-rose-100 border border-rose-200 text-rose-600 font-bold py-2.5 rounded-lg text-sm transition cursor-pointer"
              >
                일지 삭제
              </button>
            )}
            <button
              type="button"
              onClick={onClose}
              className="w-full bg-slate-150 hover:bg-slate-200 text-slate-500 py-2 rounded-lg text-sm transition cursor-pointer border border-slate-200"
            >
              취소
            </button>
          </div>
